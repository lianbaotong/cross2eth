...
func Test_RestorePrivateKeys(t *testing.T) {
	...
	privateKey, _, err := ethRelayer.GetAccount("123")
	ethRelayer.rwLock.RLock()
	assert.NotEqual(t, common.Bytes2Hex(crypto.FromECDSA(temp)), privateKey)
	ethRelayer.rwLock.RUnlock()
	require.Nil(t, err)

	privateKey, _, err = ethRelayer.GetAccount(passphrase)
	ethRelayer.rwLock.RLock()
	assert.Equal(t, common.Bytes2Hex(crypto.FromECDSA(temp)), privateKey)
	ethRelayer.rwLock.RUnlock()
	require.Nil(t, err)
	...
	privateKey, _, err = ethRelayer.GetAccount("new123")
	ethRelayer.rwLock.RLock()
	assert.Equal(t, common.Bytes2Hex(crypto.FromECDSA(temp)), privateKey)
	ethRelayer.rwLock.RUnlock()
	require.Nil(t, err)
